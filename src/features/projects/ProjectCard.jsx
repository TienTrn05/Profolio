...
export default function ProjectCard({ project, onOpenCaseStudy }) {
  return (
    <article
      className="project-card group grid overflow-hidden border-2 border-[var(--border)] bg-surface transition-[border-color,box-shadow] duration-300 hover:border-brand hover:shadow-2xl lg:grid-cols-[minmax(0,1fr)_minmax(22rem,42%)]"
      data-project={project.id}
      data-reveal
    >
      <div className="order-2 flex min-w-0 flex-col justify-center p-[clamp(2.5rem,4vw,3.5rem)] lg:order-1">
        ...
        <h3 className="mt-6 text-[clamp(2rem,2.6vw,2.65rem)] leading-[1.22] font-bold">
          {project.headline}
        </h3>
        ...
        <dl className="mt-6 border-l-[3px] border-brand bg-brand/5 py-1 pl-4">
          {project.evidence.map(([label, value]) => (
            <div
              className="grid gap-2 border-b border-[var(--border-soft)] py-3 last:border-b-0 sm:grid-cols-[7.5rem_minmax(0,1fr)]"
              key={label}
            >
        ...
        <div
          className="mt-6 border border-[var(--border-soft)] bg-page/70 p-4"
            ...
            {project.flow.map(({ icon, label, value }, index) => (
              <div className="contents" key={label}>
                {index > 0 && (
                  <Icon
                    name="arrow-right"
                    className="mx-auto size-4 shrink-0 rotate-90 text-brand sm:rotate-0"
                  />
                )}
                <div className="flex min-w-0 flex-1 items-center gap-2 border border-[var(--border)] bg-surface p-3">
